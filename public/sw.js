const CACHE="mi-balance-v2";
const BASE=new URL("./",self.registration.scope).pathname;
self.addEventListener("install",event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll([BASE,BASE+"manifest.json",BASE+"favicon.svg"]))));
